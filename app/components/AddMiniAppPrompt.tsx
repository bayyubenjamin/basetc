...
export default function AddMiniAppPrompt() {
  ...
  useEffect(() => {
    // ✅ PENTING: guard sebelum memanggil ready()
    try {
      const maybeReady = (sdk.actions as any)?.ready;
      if (typeof maybeReady === "function") {
        // panggil dan tangani promise-nya
        Promise.resolve(maybeReady()).catch(() => {});
      }
    } catch {
      // abaikan error apa pun di sini; ini hanya hint ke client
    }
  ...
  async function triggerAdd() {
    try {
      ...
      // (opsional) logging sementara; aman kalau ingin dibiarkan
      try {
        console.log(
          "[AddMiniAppPrompt] added? ",
          (sdk as any)?.context?.client?.added,
          " addMiniApp:",
          typeof add
        );
      } catch {}
      ...
      if (typeof add === "function") {
        // ⬇️ ini yang memunculkan sheet native Warpcast
        await add();
