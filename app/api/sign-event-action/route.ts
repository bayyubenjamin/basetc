...
export async function POST(req: Request) {
  try {
    ...
    if (!vault || !action || !user || nonce === undefined || !deadline) {
      return NextResponse.json(
    ...
    if (vault === "staking") {
      ...
      if (action === "stake") {
        if (!amount || lockType === undefined) {
          throw new Error("Missing amount or lockType for staking");
        }
        primaryType = "StakeAction";
        message = {
          user,
          amount, // kirim dari FE sebagai string wei, atau parse di sini sesuai kebutuhanmu
      ...
      // optional: catat poin spin ke Supabase
      if (SUPABASE_URL && SUPABASE_ANON_KEY && fid) {
        try {
          const supabase = createClient(SUPABASE_URL, SUPABASE_ANON_KEY);
          await supabase.functions.invoke("add-spin-points", {
            body: { fid },
          });
        } catch (err) {
          console.warn("supabase add-spin-points error:", err);
        }
      }
    } else {
