...
export default function Profil() {
  ...
  /** BOT-facing embed link → use /share/[addr] for dynamic OG card */
  ...
  const buildCastText = useCallback(() => {
    return CAST_PRESETS.valueForward(); // pure text (no URL)
  }, []);
  ...
  const onShareReferral = useCallback(async () => {
    ...
    try {
      await sdk.actions.composeCast({
        text: castText,
        embeds: [embedLink], // link hidden in text; card comes from OG on /share/[addr]
      });
    } catch {
      ...
      <div className="bg-neutral-800 rounded-lg p-3 space-y-3">
        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-2">
          ...
          <div className="flex items-center gap-2">
            ...
            {/* Share to Farcaster (text clean; card from /share/[addr] embeds) */}
            <button
